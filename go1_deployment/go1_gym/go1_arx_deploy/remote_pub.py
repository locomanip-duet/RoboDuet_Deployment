...
from go1_arx_deploy.lcm_types.gripper_lcmt import gripper_lcmt
    ...
    while not shutdown:
        ...
        while not shutdown:
            ...
            if np.sum(np.abs(previous_pose[0:3] - action[0:3])+0.2*np.abs(previous_pose[3:6] - action[3:6])) > arm_delta:
                arm_pose_msg.ee_pose = action[0:6]
            ...
            if np.abs(previous_gripper - action[6]) > gripper_delta:
                gripper_msg.gripper_cmd = action[6]
                previous_gripper = action[6]
                lcm_node.publish("gripper_command", gripper_msg.encode())
                print('gripper', gripper_msg.gripper_cmd)
